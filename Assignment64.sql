...
-- derived as month(tran_dt).  Same report should show 12 additional columns as one column per 
-- month and amt in the tran_dtl table show appear in the month column it belongs 
-- and remaining 11 columns should have zero as value for that row
...
CASE WHEN MONTH(tran_dt) = 6 THEN amt ELSE 0 END AS 'JUN' ,
...
CASE WHEN MONTH(tran_dt) = 10 THEN amt ELSE 0 END AS 'OCT' ,
CASE WHEN MONTH(tran_dt) = 11 THEN amt ELSE 0 END AS 'NOV' ,
CASE WHEN MONTH(tran_dt) = 12 THEN amt ELSE 0 END AS 'DEC' 
FROM tran_dtl td ;
